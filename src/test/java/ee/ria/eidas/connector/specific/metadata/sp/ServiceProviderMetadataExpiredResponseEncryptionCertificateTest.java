...
public class ServiceProviderMetadataExpiredResponseEncryptionCertificateTest extends ServiceProviderTest {

    @BeforeAll
    static void beforeAll() {
        startServiceProviderMetadataServer();
        startServiceProvider1MetadataServer("sp1-expired-response-encryption-cert.xml");
    }
    ...
    @Order(2)
    @SneakyThrows
    void responseAssertionEncryptionFailsWhen_InvalidMetadataState() {
        ...
        assertThrows(EncryptionException.class, () -> sp1Metadata.encrypt(assertion));
    }
    ...
    @Order(5)
    @SneakyThrows
    void responseAssertionEncryptionSucceedsWhen_ValidMetadataState() {
        ServiceProviderMetadata sp1Metadata = serviceProviderMetadataRegistry.getByEntityId(SP_1_ENTITY_ID);
        assertNotNull(sp1Metadata);
        assertTrue(sp1Metadata.isUpdatedAndValid());
        AssertionBuilder assertionBuilder = new AssertionBuilder();
        Assertion assertion = assertionBuilder.buildObject();
        EncryptedAssertion encryptedAssertion = sp1Metadata.encrypt(assertion);
